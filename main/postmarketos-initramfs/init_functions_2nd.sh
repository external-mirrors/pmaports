...
handle_immutable_boot() {
    local boot_device root_partition
	local usrhash="$1"
	...
	if [ -n "$partition" ] && mount_and_check_populated "$partition"; then
	...
	# Mount /usr for all immutable installs
