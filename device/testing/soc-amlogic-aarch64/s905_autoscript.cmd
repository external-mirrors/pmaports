...
setenv pmos_env_load 'fatload ${pmos_devtype} ${pmos_devnum} ${pmos_env_addr} environ.txt && env import -t ${pmos_env_addr} ${filesize}'
setenv pmos_try_uimage 'if fatload ${pmos_devtype} ${pmos_devnum} ${loadaddr} uImage; then fatload ${pmos_devtype} ${pmos_devnum} ${dtb_mem_addr} ${devicetree} && bootm ${loadaddr} - ${dtb_mem_addr}; fi'
